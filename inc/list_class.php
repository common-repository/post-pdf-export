...
class template_list extends WP_List_Table {
	...
	*/
 	function column_default( $item, $column_name ) {
		...
		switch( $column_name ) { 
			case 'template_name': return stripslashes($item->template_name); break;
			case 'template_description': return stripslashes($item->template_description); break;
			case 'create_by': return $user_name; break;
			case 'create_date': return date( 'd-m-Y' , strtotime( $item->create_date ) ); break;
			default:
	...
	*/
	function column_template_name($item){
		...
		$arr_params = array ( 'template' => $item->template_id , 'pdfex_action' => 'edit' );
		$editlink  = add_query_arg( $arr_params );
		
		$arr_params = array ( 'template' => $item->template_id , 'pdfex_action' => 'delete' );
	...
	*/
	function column_cb($item) {
        ...
        return sprintf(
            '<input type="checkbox" name="template[]" value="%s" />', $item->template_id
        );  
        ...
        if( 'delete'===$this->current_action() ) {
        	...
        	if( count( $_POST['template'] ) > 0 ) {
	            ...
	            foreach($_POST['template'] as $template) {
	                ...
	                $pdfex_core->delete_template( $template );
	...
	*/
	function process_link_action(){
		...
		if( isset( $_GET['pdfex_action'] ) && $_GET['pdfex_action']=='delete' ) {
			$pdfex_core->delete_template(  $_GET['template'] );
		}
	...
	*/
	function prepare_items() {
		...
		$query = "SELECT * FROM ".$wpdb->prefix."pdfex_template";
