...
class shortcode {
	...
	function __construct(){
		...
		if (is_admin() || isset( $_GET['pdfex_dl'] ) || isset( $_GET['pdfex_post_dl'] ) ) {

			$this->register_tempalte_shortcodes();

		} else {

			add_shortcode( 'pdfex', array( &$this , 'apply_download_button') );
	...
	*/
	...
	public function register_tempalte_shortcodes() {
		...
		add_shortcode( 'site_title', array( &$this , 'site_title_func') );

		add_shortcode( 'site_tagline', array( &$this , 'site_tagline_func') );

		add_shortcode( 'site_url', array( &$this , 'site_url_func') );

		add_shortcode( 'date_today', array( &$this , 'date_today_func') );

		add_shortcode( 'from_date', array( &$this , 'from_date_func') );

		add_shortcode( 'to_date', array( &$this , 'to_date_func') );

		add_shortcode( 'categories', array( &$this , 'categories_func') );

		add_shortcode( 'post_count', array( &$this , 'post_count_func') );

		add_shortcode( 'loop', array( &$this , 'loop_func') );

		

		add_shortcode( 'title', array( &$this , 'title_func') );

		add_shortcode( 'excerpt', array( &$this , 'excerpt_func') );

		add_shortcode( 'content', array( &$this , 'content_func') );

		add_shortcode( 'permalink', array( &$this , 'permalink_func') );

		add_shortcode( 'date', array( &$this , 'date_func') );

		add_shortcode( 'author', array( &$this , 'author_func') );

		add_shortcode( 'author_photo', array( &$this , 'author_photo_func') );

		add_shortcode( 'author_description', array( &$this , 'author_description_func') );

		add_shortcode( 'status', array( &$this , 'status_func') );

		add_shortcode( 'featured_image', array( &$this , 'featured_image_func') );

		add_shortcode( 'category', array( &$this , 'category_func') );

		add_shortcode( 'tags', array( &$this , 'tags_func') );

		add_shortcode( 'comments_count', array( &$this , 'comments_count_func') );
	...
	*/
	...
	public function loop_func(){
		...
		if( count( $pdfex_core->posts ) > 0 ) {

			foreach( $pdfex_core->posts as $post ) {

				$this->single = $post;

				$item .= $pdfex_core->filter_shortcodes('loop');
